...
const RuxSelect = class {
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("label", { id: labelId, htmlFor: inputId, "aria-hidden": this.hasLabel ? 'false' : 'true' }, index.h("span", { class: { hidden: !this.hasLabel } }, index.h("slot", { onSlotchange: this._handleLabelSlotChange, name: "label" }, label))), index.h("select", { class: 'rux-select ' + (invalid ? 'rux-select-invalid' : ''), ref: (el) => (this.selectEl = el), id: inputId, disabled: disabled, required: required, name: name, onChange: (e) => this._onChange(e), onBlur: this._onBlur }), index.h("div", { "aria-hidden": "true", class: "hidden", ref: (el) => (this.slotContainer = el) }, index.h("slot", { onSlotchange: this._handleSlotChange })), index.h(FormFieldMessage.FormFieldMessage, { errorText: this.errorText, helpText: this.helpText })));
  }
